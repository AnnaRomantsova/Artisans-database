 ...
 //список разделов мастера
  ...
  global $db;
         ...
         while ($r1->next_row()) {
            unset($sub);
            $sub = new outTree();
            $r1->addFields($sub,$ar=array('about'));
  ...
  function check_review_rights(&$main,$user_id,$type,$id_what=0){
       ...
       $rev=true;
       //echo $user_id;
       if (!$user_id>0) { $rev = false; $err_message='Отклики и комментарии могут оставлять только зарегистрированные пользователи.';
       //заказы
       } else if ($type==1) {
            //проверка на свой заказ
            $r=new Select($db,"select * from zakaz where id=$id_what");
            $r->next_row();
            if ($r->result('id_user') == $user_id) {$rev = false;}

            //не мастер
            else if (is_master($user_id) ==false) {$rev = false; $err_message='Отзывы могут оставлять только зарегистрированные мастера.';}
            ...
            else {$r1 = new Select($db,"select * from zakaz where id=$id_what");
               $r1->next_row();
       ...
       if ($rev==true) $main->addField( 'rev_true','');
          else {$main->addField( 'rev_false',''); $main->addField( 'err_mess',$err_message);};
