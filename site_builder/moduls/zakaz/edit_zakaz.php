<?php
 ...
 if ($_GET['id']>0) {
       ...
       if ($_POST['save']>0) {
           ...
           $r = new Select($db,"update zakaz set name='$name',id_city=$city,price='$price',time=$time,date_before=$date_before,about='$about',pabl=0
                                    where id = $_GET[id]");
           ...
           $message = 'ƒанные сохранены. ¬аш заказ отобразитс€ на сайте после проверки модератором.';
      };
      ...
      if ($_GET['new']>0) $message = 'ƒанные сохранены. ¬аш заказ отобразитс€ на сайте после проверки модератором.'; 
