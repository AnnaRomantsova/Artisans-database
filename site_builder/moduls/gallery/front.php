<?php
 ...
 //$where  = $razdel;
 //echo $where;
 $query = "select distinct(g.id) as id,g.* from galery g,users u, user_types ut
             where g.pabl=1 and g.id_user=u.id and ut.id_user=u.id $city and ut.id_user=u.id $razdel order by rand()";
 ...
 while ($r->next_row()) {
        ...
        unset($sub);
        $sub = new outTree();
        $r->addFields($sub,$ar=array('id','about'));
